...
void C_EnemyB::Update()
{
	...
	if (m_bFlg)
	{
		...
		if (frame >= 50)
		{
			...
			tempBullet->SetEnemyColor(EnemyBlue);

			tempBullet->Shot({ m_pos.x - 10, m_pos.y }, 5);
...
void C_EnemyB::Draw()
{
	...
	DrawImg(m_spawnMat, &m_C_kasanTex, { 64,0,64,64 }, 1.0f);	//色付きの加算合成
	D3D.SetBlendState(BlendMode::Alpha);

	switch (m_enemyAnimeCnt)
	{
	case 0:
		DrawImg(m_mat, &m_tex, { 0,64,64,64 }, 1.0f);
		break;
	case 1:
		DrawImg(m_mat, &m_tex, { 64,64,64,64 }, 1.0f);
		break;
	case 2:
		DrawImg(m_mat, &m_tex, { 128,64,64,64 }, 1.0f);
		break;
	case 3:
		DrawImg(m_mat, &m_tex, { 192,64,64,64 }, 1.0f);
		break;
	case 4:
		DrawImg(m_mat, &m_tex, { 256,64,64,64 }, 1.0f);
		break;
	}
	...
	{
	case 0:
		DrawImg(m_spawnMat, &m_spawnTex, { 0,64,64,64 }, 1.0f);
		break;
	case 1:
		DrawImg(m_spawnMat, &m_spawnTex, { 64,64,64,64 }, 1.0f);
		break;
	case 2:
		DrawImg(m_spawnMat, &m_spawnTex, { 128,64,64,64 }, 1.0f);
		break;
	case 3:
		DrawImg(m_spawnMat, &m_spawnTex, { 192,64,64,64 }, 1.0f);
		break;
	case 4:
		DrawImg(m_spawnMat, &m_spawnTex, { 256,64,64,64 }, 1.0f);
		break;
	}
	D3D.SetBlendState(BlendMode::Alpha);
}
